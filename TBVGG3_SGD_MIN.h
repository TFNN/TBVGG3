...
--------------------------------------------------
    ...
    Comments;
        ...
        `LEARNING_RATE` and `GRADIENT_GAIN` hyperparameters.
    ...
    Network size:
        (3x28x28)+(8x3x9)+(16x32x9)+(32x64x9)+(8x28x28)+
        (8x14x14)+(16x14x14)+(16x7x7)+(32x7x7)+(8x28x28)+
        (16x14x14)+(32x7x7)+(8+64+128)+(8+64+128) = 50,312 floats
        50312*4 = 20,124 bytes = 19.6 kilobytes
*/
...
struct
{
    ...
    //       d,  y,  x
    float o1[8 ][28][28];
        float p1[8 ][14][14]; // pooled
    float o2[16][14][14];
        float p2[16][7][7];   // pooled
    float o3[32][7][7];
...
float TBVGG3_Process(TBVGG3_Network* net, const float input[3][28][28], const TBVGG3_LEARNTYPE learn)
{
    ...
    // convolve input with 8 filters
    for(uint i = 0; i < 8; i++) // num filter
    {
    ...
    // max pool the output
    TBVGG3_2x2MaxPool(8, 28, net->o1, net->p1);

    // convolve output with 16 filters
    for(uint i = 0; i < 16; i++) // num filter
    {
        for(uint j = 0; j < 14; j++) // height
        {
            ...
            {
                net->o2[i][j][k] = TBVGG3_3x3Conv(8, 14, net->p1, j, k, net->l2f[i], net->l2fb[i]);
            }
        }
    }

    // max pool the output
    TBVGG3_2x2MaxPool(16, 14, net->o2, net->p2);
    ...
    for(uint i = 0; i < 32; i++) // num filter
    {
        for(uint j = 0; j < 7; j++) // height
        {
            ...
            {
                net->o3[i][j][k] = TBVGG3_3x3Conv(16, 7, net->p2, j, k, net->l3f[i], net->l3fb[i]);
            }
        }
    }
    ...
    else
    {
        ...
        for(uint i = 0; i < 32; i++) // num filter
        {
            net->l3fbe[i][0] = GRADIENT_GAIN * g0; // bias error
            ...
            for(uint j = 0; j < 7; j++) // height
            {
                ...
                {
                    // set error
                    net->e3[i][j][k] = GRADIENT_GAIN * TBVGG3_RELU_D(net->o3[i][j][k]) * g0;
                    ...
                    l3er += GRADIENT_GAIN * net->l3fb[i][0] * g0;
                }
            }
        }
        ...
        for(uint i = 0; i < 16; i++) // num filter
        {
            net->l2fbe[i][0] = GRADIENT_GAIN * l3er; // bias error
            ...
            for(uint j = 0; j < 14; j++) // height
            {
                ...
                {
                    // set error
                    net->e2[i][j][k] = GRADIENT_GAIN * TBVGG3_RELU_D(net->o2[i][j][k]) * l3er;

                    // every output error gradient for every filter weight :: per filter
                    for(uint d = 0; d < 8; d++) // depth
                        for(uint w = 0; w < 9; w++) // weight
                            l2er += net->l2f[i][d][w] * net->e2[i][j][k];
                    l2er += GRADIENT_GAIN * net->l2fb[i][0] * l3er;
                }
            }
        }
        ...
        for(uint i = 0; i < 8; i++) // num filter
        {
            net->l1fbe[i][0] = GRADIENT_GAIN * l2er; // bias error

            for(uint j = 0; j < 28; j++) // height
                for(uint k = 0; k < 28; k++) // width
                    net->e1[i][j][k] = GRADIENT_GAIN * TBVGG3_RELU_D(net->o1[i][j][k]) * l2er; // set error
        }
        ...
        for(uint i = 0; i < 16; i++) // num filter
        {
            ...
            for(uint j = 0; j < 14; j++) // height
                for(uint k = 0; k < 14; k++) // width
                    TBVGG3_3x3ConvB(8, 14, net->o1, net->e2, j, k, net->l2f[i]);
        }
        ...
        for(uint i = 0; i < 32; i++) // num filter
        {
            ...
            for(uint j = 0; j < 7; j++) // height
                for(uint k = 0; k < 7; k++) // width
                    TBVGG3_3x3ConvB(16, 7, net->o2, net->e3, j, k, net->l3f[i]);
        }
