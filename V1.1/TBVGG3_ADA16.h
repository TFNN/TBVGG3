...
float TBVGG3_Process(TBVGG3_Network* net, const float input[3][28][28], const TBVGG3_LEARNTYPE learn)
{
    ...
    // filter momentum's
    float l1fm[16][3 ][9]={0};
    float l2fm[32][16][9]={0};
    float l3fm[64][32][9]={0};
    ...
    for(uint i = 0; i < 32; i++) // num filter
    {
        for(uint j = 0; j < 14; j++) // height
        {
            ...
            {
                o2[i][j][k] = TBVGG3_3x3Conv(16, 14, p1, j, k, net->l2f[i], net->l2fb[i]);
            }
        }
    }

    // max pool the output
    TBVGG3_2x2MaxPool(32, 14, o2, p2);
    ...
    for(uint i = 0; i < 64; i++) // num filter
    {
        for(uint j = 0; j < 7; j++) // height
        {
            ...
            {
                o3[i][j][k] = TBVGG3_3x3Conv(32, 7, p2, j, k, net->l3f[i], net->l3fb[i]);
            }
        }
    }
    ...
    else
    {
        ...
        for(uint i = 0; i < 32; i++) // num filter
        {
            for(uint j = 0; j < 14; j++) // height
                for(uint k = 0; k < 14; k++) // width
                    TBVGG3_3x3ConvB(16, 14, o1, e2, j, k, net->l2f[i], l2fm[i], net->l2fb[i], l2fbm[i]);
        }
        ...
        for(uint i = 0; i < 64; i++) // num filter
        {
            for(uint j = 0; j < 7; j++) // height
                for(uint k = 0; k < 7; k++) // width
                    TBVGG3_3x3ConvB(32, 7, o2, e3, j, k, net->l3f[i], l3fm[i], net->l3fb[i], l3fbm[i]);
        }
